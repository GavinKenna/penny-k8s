      ...
      onConnect: () => {
        ...
        stompClient.subscribe("/topic/configmaps", (message) => {
          try {
            ...
            if (event.eventType === "ADDED") {
              if (
                !configMaps.value.some((cm) => cm.name === event.configMap.name)
              ) {
              ...
              configMaps.value = configMaps.value.filter(
                (cm) => cm.name !== event.configMap.name,
              );
            }
